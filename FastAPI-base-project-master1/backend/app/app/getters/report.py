...
def get_report_image(report_image: ReportImage) -> GettingReportImage:
    ...
    return transform(db_obj=report_image, target_schema=GettingReportImage)

def get_report(report: Report) -> GettingReport:
    # return transform(db_obj=order, target_schema=GettingOrder, user=get_user(order.user) if order.user is not None else None)
    return transform(
        db_obj=report, 
        target_schema=GettingReport,
        imgs=[get_report_image(img) for img in report.report_images]
    )